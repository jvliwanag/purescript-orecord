...
type O
  = ORecord ( i :: Int, s :: String ) ( b :: Boolean, n :: Number )
...
main = do
  ...
  assert' "gets required member"
    $ getRequired i_ sample
    == 10
  assert' "gets defined optional member"
    $ getOptional b_ sample
    == Just true
  assert' "gets undefined optional member"
    $ getOptional n_ sample
    == Nothing
  assert' "get after set required"
    $ getRequired i_ (setRequired i_ 20 sample)
    == 20
  assert' "get after set optional"
    $ getOptional b_ (setOptional b_ (Just false) sample)
    == (Just false)
  assert' "get after unset optional"
    $ getOptional b_ (setOptional b_ Nothing sample)
    == Nothing
  assertFalse' "unsetting removes key"
    $ sample
    # setOptional b_ Nothing
    # hasKey "b"
